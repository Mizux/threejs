...
class TransformControls extends Object3D {
  ...
  updateMatrixWorld() {
    ...
    if (this.object !== undefined) {
      ...
      this.object.matrixWorld.decompose(this.worldPosition,
                                        this.worldQuaternion, this._worldScale);
    ...
    this.camera.matrixWorld.decompose(this.cameraPosition,
                                      this.cameraQuaternion, this._cameraScale);
  ...
  pointerMove(pointer) {
    ...
    if (mode === 'translate') {
      ...
      if (space === 'local' && axis !== 'XYZ') {
        ...
        this._offset.applyQuaternion(this._parentQuaternionInv)
            .divide(this._parentScale);
      }
      ...
      const ROTATION_SPEED =
          20 / this.worldPosition.distanceTo(
      ...
      if (space === 'local' && axis !== 'E' && axis !== 'XYZE') {
        ...
        object.quaternion
            .multiply(_tempQuaternion.setFromAxisAngle(this.rotationAxis,
                                                       this.rotationAngle))
            .normalize();
  ...
  setRotationSnap(rotationSnap) { this.rotationSnap = rotationSnap; }

  setScaleSnap(scaleSnap) { this.scaleSnap = scaleSnap; }
...
class TransformControlsGizmo extends Object3D {
  ...
  constructor() {
    ...
    // Special geometry for transform helper. If scaled with position vector it
    // spans from [0,0,0] to position
    ...
    // Gizmo definitions - custom hierarchy definitions for setupGizmo()
    // function
    ...
    const helperTranslate = {
      START : [ [
        new Mesh(new OctahedronGeometry(0.01, 2), matHelper), null, null, null,
        'helper'
      ] ],
      END : [ [
        new Mesh(new OctahedronGeometry(0.01, 2), matHelper), null, null, null,
      ...
      X : [ [
        new Line(lineGeometry, matHelper.clone()), [ -1e3, 0, 0 ], null,
        [ 1e6, 1, 1 ], 'helper'
      ] ],
      Y : [ [
        new Line(lineGeometry, matHelper.clone()), [ 0, -1e3, 0 ],
        [ 0, 0, Math.PI / 2 ], [ 1e6, 1, 1 ], 'helper'
      ] ],
      Z : [ [
        new Line(lineGeometry, matHelper.clone()), [ 0, 0, -1e3 ],
        [ 0, -Math.PI / 2, 0 ], [ 1e6, 1, 1 ], 'helper'
      ] ]
    };
    ...
    const gizmoRotate = {
      ...
      Y : [ [
        new Mesh(CircleGeometry(0.5, 0.5), matGreen), null,
        [ 0, 0, -Math.PI / 2 ]
      ] ],
      Z : [ [
      ...
      E : [ [
        new Mesh(CircleGeometry(0.75, 1), matYellowTransparent), null,
        [ 0, Math.PI / 2, 0 ]
      ] ]
    };
    ...
    const gizmoScale = {
      ...
      XYZ : [
        [ new Mesh(new BoxGeometry(0.1, 0.1, 0.1),
    ...
    function setupGizmo(gizmoMap) {
      ...
      for (const name in gizmoMap) {
        ...
        for (let i = gizmoMap[name].length; i--;) {
          ...
          // name and tag properties are essential for picking and updating
          // logic.
          object.name = name;
          object.tag = tag;
  ...
  // updateMatrixWorld will update transformations and appearance of individual
  // handles

  updateMatrixWorld(force) {

    const space = (this.mode === 'scale')
                      ? 'local'
    ...
    for (let i = 0; i < handles.length; i++) {
      ...
      if (handle.tag === 'helper') {
        ...
        if (handle.name === 'AXIS') {
          ...
          if (this.axis === 'X') {
            ...
            if (Math.abs(_alignVector.copy(_unitX)
          ...
          if (this.axis === 'Y') {
            ...
            if (Math.abs(_alignVector.copy(_unitY)
                             .applyQuaternion(quaternion)
                             .dot(this.eye)) > 0.9) {
          ...
          if (this.axis === 'Z') {
            ...
            if (Math.abs(_alignVector.copy(_unitZ)
                             .applyQuaternion(quaternion)
                             .dot(this.eye)) > 0.9) {
          ...
          _tempVector.set(1e-10, 1e-10, 1e-10)
      ...
      if (this.mode === 'translate' || this.mode === 'scale') {
        ...
        if (handle.name === 'X') {
          ...
          _tempQuaternion.multiplyQuaternions(_tempQuaternion2,
                                              _tempQuaternion);
      ...
      handle.visible =
          handle.visible && (handle.name.indexOf('E') === -1 ||
...
class TransformControlsPlane extends Mesh {
  ...
  updateMatrixWorld(force) {
    ...
    _v1.copy(_unitX).applyQuaternion(space === 'local' ? this.worldQuaternion
                                                       : _identityQuaternion);
    _v2.copy(_unitY).applyQuaternion(space === 'local' ? this.worldQuaternion
                                                       : _identityQuaternion);
    _v3.copy(_unitZ).applyQuaternion(space === 'local' ? this.worldQuaternion
                                                       : _identityQuaternion);
