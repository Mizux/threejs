...
class OrbitControls extends EventDispatcher {
  ...
  constructor(object, domElement) {
    ...
    // If set, the interval [ min, max ] must be a sub-interval of [ - 2 PI, 2
    // PI ], with ( max - min < 2 PI )
    ...
    const pan = function() {
      ...
      return function pan(deltaX, deltaY) {
        ...
        if (scope.object.isPerspectiveCamera) {
          ...
          // we use only clientHeight here so aspect ratio does not distort
          // speed
    ...
    function getSecondPointerPosition(event) {
      ...
      const pointer = (event.pointerId === pointers[0].pointerId) ? pointers[1]
                                                                  : pointers[0];
